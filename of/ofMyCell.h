...
public:
     ...
     *
     * 	\param index: value of the RGBA integer color
     */
    void setColor(ids *c);
...
template <  class _Traits> void ofMyCell< _Traits>::setColor(ids *c)
{
    int i;
    for(i=0;i<4;i++)
        color[i]=c[i];
...
template <class _Traits> IDS ofMyCell<_Traits>::getColorR()
{
    return color[0];
}

template <class _Traits> IDS ofMyCell<_Traits>::getColorG()
{
    return color[1];
}

template <class _Traits> IDS ofMyCell<_Traits>::getColorB()
{
    return color[2];
}

template <class _Traits> IDS ofMyCell<_Traits>::getColorA()
{
    return color[3];
}
