...
scrInteractor::scrInteractor(int width, int height)
{
    ...
    // glfw window creation
        // --------------------
        this->window = glfwCreateWindow(this->rec_screen.Width(), this->rec_screen.Height(), "Visualizador", NULL, NULL);
        if (window == NULL)
        {
...
void scrInteractor::setDraw(void (* func)(const Shader&))
{
...
/*===================================================================*/
void scrInteractor::MouseMotion(GLFWwindow* window,int x, int y) /* é acessada com o movimento do mouse, com o botão clicado*/
{
...
/*===================================================================*/
void scrInteractor::Mouse(GLFWwindow* window,int button, int state, int x, int y) /* coordena */
{
	int mod_keys;
...
// static GLint my;
    ...
    if (state == GLFW_PRESS)
	{
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT)
		{
            ...
            if (mod_keys == GLFW_KEY_LEFT_SHIFT )
			{
				double point [3];
				char aux_label[7];
				char btn_label[25];
...
/*===================================================================*/
void scrInteractor::scrInteractorMainLoop()
{
...
void scrInteractor::processInput()
{
    ...
    if (glfwGetKey(window, GLFW_KEY_RIGHT) == GLFW_PRESS)
    {
        ...
        cameraInitialPos=camera->Position;
    ...
    {
        cameraPos =cameraInitialPos = camerareferenceInitialPos;
        //camerareferenceInitialPos=cameraInitialPos;
        //cameraFront= cameraInitialFront;
        planeRotXYZ = glm::vec3(0.0f);
        camera->Position=cameraPos;
        camera->WorldUp=cameraInitialUp;
        camera->Yaw=-90.0f;
        camera->Pitch=0.0f;
...
void scrInteractor::mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    if (firstMouse)
    {
        lastX = xpos;
...
void scrInteractor::Calc_Rotation()
{
    ...
    TrackBall temp_trackball(sx, sy, ex, ey, false, false);



	this->trackball.Add_Quats(temp_trackball.getQuats());
    this->trackball.setAngPhi(temp_trackball.getAngPhi());
...
void scrInteractor::ScreenToPoint(auxPoint<int> mouse, double point[3])
{
	...
	glPushMatrix();
        ...
        //gluUnProject(mouse.x, my, mz,model_view_matrix, projection_matrix, viewport,	&point[0], &point[1], &point[2]);
	glPopMatrix();
...
/*===================================================================*/
void scrInteractor::Change_BG_Color(GLFWwindow* window) /* atualiza a cor do BG*/
{
   //glutSetWindow(this->main_winid);
...
void scrInteractor::setUserBGColor() /*onde o usuario seta a cor do BG*/
{
   ...
   this->bg_color[0] = 0.2; /*r*/
   this->bg_color[1] = 0.3; /*g*/
   this->bg_color[2] = 0.3; /*b*/
...
//void scrInteractor:: framebuffer_size_callback(GLFWwindow* window, int width, int height)
